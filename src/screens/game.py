...
class GameScreen(Screen):
    def __init__(self, app: "GameApp") -> None:
        ...
        self.root_panel_window = self.canvas.create_window(0, 0, window=self.root_panel, anchor="center")
    ...
    def _build_question_block(self) -> None:
        ...
        self.question_label.grid(row=0, column=1, sticky="ew")
    ...
    def on_resize(self, width: int, height: int) -> None:
        ...
        scale = min(draw_w / 1280, draw_h / 720)

        panel_width = self._clamp(int(draw_w * 0.78), 760, 1140)
        panel_center_x = draw_w // 2
        panel_center_y = draw_h // 2

        self.canvas.coords(self.background_base_id, 0, 0, draw_w, draw_h)
        self.canvas.coords(self.background_tint_id, 0, 0, draw_w, draw_h)

        self.canvas.coords(self.root_panel_window, panel_center_x, panel_center_y)
        self.canvas.itemconfigure(self.root_panel_window, width=panel_width)

        header_font = self._clamp(int(18 * scale), 12, 18)
        question_font = self._clamp(int(22 * scale), 14, 22)
        answer_font = self._clamp(int(14 * scale), 10, 14)
        badge_font = self._clamp(int(24 * scale), 14, 24)
        feedback_font = self._clamp(int(13 * scale), 10, 13)

        self.menu_btn.configure(font=("Arial", self._clamp(int(14 * scale), 10, 14), "bold"))
        self.progress_btn.configure(font=("Arial", header_font, "bold"))
        self.amount_btn.configure(font=("Arial", header_font, "bold"))
        self.question_prefix_label.configure(font=("Arial", question_font, "bold"))
        self.question_label.configure(
            font=("Arial", question_font, "bold"),
            wraplength=max(320, int(panel_width * 0.72)),
        )
        self.feedback_label.configure(font=("Arial", feedback_font, "bold"))

        for hint in [self.hint_bird, self.hint_5050, self.hint_2x]:
            hint.itemconfigure(2, font=("Arial", self._clamp(int(23 * scale), 14, 23), "bold"))

        for badge_canvas, _oval_id, text_id in self.answer_badges.values():
            badge_canvas.itemconfigure(text_id, font=("Arial", badge_font, "bold"))

        for button in self.answer_buttons.values():
            button.configure(font=("Arial", answer_font, "bold"))
