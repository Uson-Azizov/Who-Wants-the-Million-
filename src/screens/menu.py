...
class MenuScreen(Screen):
    ...
    def _build_buttons(self) -> None:
        ...
        self.play_btn = GlassButton(self.canvas, text="Играть", command=self.app.start_game, **style)
        self.settings_btn = GlassButton(self.canvas, text="Настройки", command=self.app.open_settings, **style)
        self.exit_btn = GlassButton(self.canvas, text="Выход", command=self.app.quit, **style)
    ...
    def on_resize(self, width: int, height: int) -> None:
        ...
        title_size = self._clamp(int(58 * scale), 30, 58)
        subtitle_size = self._clamp(int(18 * scale), 12, 18)
        button_font_size = self._clamp(int(20 * scale), 13, 20)
        records_font_size = self._clamp(int(13 * scale), 10, 13)
        logo_font_size = self._clamp(int(18 * scale), 10, 18)

        title_y = int(height * 0.08)
        self.canvas.itemconfigure(self.title_main_id, font=("Arial", title_size, "bold"))
        self.canvas.itemconfigure(self.title_highlight_id, font=("Arial", title_size, "bold"))
        self.canvas.itemconfigure(self.title_main_glow_id, font=("Arial", title_size, "bold"))
        self.canvas.itemconfigure(self.title_highlight_glow_id, font=("Arial", title_size, "bold"))
        self.canvas.itemconfigure(self.logo_text_id, font=("Arial", logo_font_size, "bold"))
        self.canvas.itemconfigure(self.status_id, font=("Arial", subtitle_size, "bold"))
        self.canvas.itemconfigure(self.hint_id, font=("Arial", self._clamp(int(12 * scale), 9, 12)))
        ...
        logo_size = self._clamp(int(min(width, height) * 0.20), 120, 220)
        logo_top = int(height * 0.24)
        self.canvas.coords(
            self.logo_outer_id,
        ...
        button_width = self._clamp(int(width * 0.34), 260, 520)
        button_center_x = center_x
        start_y = int(height * 0.58)
        gap = self._clamp(int(height * 0.10), 58, 86)

        for button in [self.play_btn, self.settings_btn, self.exit_btn]:
            button.configure(font=("Arial", button_font_size, "bold"))
        self.records_btn.configure(font=("Arial", records_font_size, "bold"))
        ...
        self.canvas.coords(self.records_btn_id, button_center_x, start_y + gap * 3)

        for item in [self.play_btn_id, self.settings_btn_id, self.exit_btn_id]:
            self.canvas.itemconfigure(item, width=button_width)

        self.canvas.itemconfigure(self.records_btn_id, width=self._clamp(int(button_width * 0.58), 180, 280))

        self.canvas.itemconfigure(self.status_id, text=self.app.status_text_var.get())
        self.canvas.coords(self.status_id, center_x, int(height * 0.92))
