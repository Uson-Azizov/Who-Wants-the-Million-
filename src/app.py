...
class MillionaireGameApp:
    def __init__(self) -> None:
        ...
        self.database = DatabaseClient(SQLITE_DB_PATH)
        db_health = self.database.initialize()
        self.db_status_var = tk.StringVar(value=db_health.message)

        repository = QuestionRepository(QUESTIONS_FILES, self.database)
    ...
    def check_database(self) -> None:
        ...
        self.status_text_var.set(health.message if health.connected else "Ошибка подключения SQLite")
    ...
    def save_game_result(self, difficulty: Difficulty | str, is_win: bool) -> None:
        ...
        if saved:
            self.db_status_var.set("Результат игры сохранен в SQLite")
            return

        if self.database.enabled:
            message = self.database.last_error or "Не удалось сохранить результат в SQLite"
            self.db_status_var.set(message)
